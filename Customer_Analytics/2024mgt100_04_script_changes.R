...
cust_dat <- read_csv("/Users/edgarhiramadelortiz/Desktop/Econometrics/Customer Analytics/mgt100-data/smartphone_customer_data.csv", show_col_types = F)
n <- nrow(cust_dat)
...
phone_dat <- read_csv("/Users/edgarhiramadelortiz/Desktop/Econometrics/Customer Analytics/mgt100-data/phone_dat.csv", show_col_types = F)
...
sub1$choice <- as.logical(sub1$choice == "Yes")  # Assuming "Yes" means TRUE
sub2$choice <- as.logical(sub2$choice == "Yes")
sub3$choice <- as.logical(sub3$choice == "Yes")

sub1$choice <- factor(sub1$choice, levels = c("Yes", "No")) %>% as.logical()
sub2$choice <- factor(sub2$choice, levels = c("Yes", "No")) %>% as.logical()
sub3$choice <- factor(sub2$choice, levels = c("Yes", "No")) %>% as.logical()
...
save(sub1, sub2, sub3, mdat1, mdat2, mdat3, file = "/Users/edgarhiramadelortiz/Desktop/Econometrics/Customer Analytics/mgt100-data/mnl_datasets.RData")
...
nrow(subsetted_data)  # Check the number of rows
